# CLAUDE.md — Claude Agent Workflow
...
|---|-------|-------------------------|
| 1 | PRODUCT-OWNER | Framework vision, template content decisions, versioning strategy, conversion path design. Recommends approvals to operator. |
| 2 | TECH-LEAD | Template quality, shell script review, markdown consistency, packaging validation. Convention enforcement, contract compliance. |
| 3 | TECH-WRITER | Template documentation clarity, conversion guides, README content, user-facing docs. Audience: Claude Code power users. |
...
|-----------|---------------------|
| Agent definitions (`global/agents/`) | TECH-LEAD |
| Command definitions (`global/commands/`) | TECH-LEAD |
| Template repo files (`template/repo/`) | PRODUCT-OWNER + TECH-LEAD |
| Conversion docs (`conversion/`) | TECH-WRITER |
| Shell scripts (`scripts/`) | TECH-LEAD |
| CI/CD (`.github/workflows/`) | TECH-LEAD |
...
Scopes: template, agents, commands, scripts, ci, docs, conversion
...
**Type:** Developer tooling / framework template
**What:** Orchestration framework for coordinating Claude Code AI agents across software projects. Provides agent definitions, slash commands, project templates, and conversion tooling.
**Beachhead:** Claude Code power users running multi-agent workflows
**Principle:** Convention over configuration — provide sensible defaults that work out of the box

## Important Context

- This is a framework/template project, not application code. All "code" is Markdown and shell scripts.
- Must dogfood its own orchestration process (this repo uses the framework to manage the framework).
- The `/convert` command expects a specific zip structure: `global/`, `repo/`, `CONVERSION.md` at root.
- Agent definitions are global (`~/.claude/agents/`) while project files are per-repo.
- `global/` is the canonical source for agents and commands. `template/` holds per-repo templates. `scripts/package.sh` assembles them into a distributable zip at build time.
- Other projects running this framework may post optimization findings back to this repo's TASKS.md (see Cross-Repo Feedback).

## What NOT to Build (MVP)

- GUI or web interface for managing the framework
- Automated project scaffolding CLI beyond `/convert`
- Agent marketplace or registry
- Automated version migration (manual conversion docs are sufficient for v1)
- Integration with non-Claude AI systems
- Runtime agent orchestration daemon — this is a file-based convention system

## Success Metrics (Pilot)

- Successfully convert one existing CTO-orchestrator project using `/convert` with the packaged zip
- Package and distribute template that installs cleanly on a fresh project
- All 23 agent definitions, 4 commands, and all template files pass markdown linting
- Shell scripts pass shellcheck
- Worktree isolation section included and validated in at least one real parallel dispatch
- Cross-repo feedback loop validated (at least one external project posts back here)
