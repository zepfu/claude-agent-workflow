...
> **Defines how the PROJECT-COORDINATOR (main session) syncs work to GitHub Issues and Projects.**
> GitHub is the external-facing progress dashboard. Internal files (PROJECT_LOG.md, TASKS.md, agent-logs/) are the working memory. This file bridges the two.
> **You (PROJECT-COORDINATOR) own all GitHub operations.** No other agent touches GitHub directly.
...
One GitHub Milestone per phase. Gate check items from PHASES.md become issues with the `type:gate-check` label assigned to the milestone.
...
You handle all GitHub operations. TECH-LEAD and PRODUCT-OWNER report events to you; you translate them into GitHub state.
...
### Operator → TASKS.md
Operator edits TASKS.md. You pick it up on session start, triage with PRODUCT-OWNER, route to TECH-LEAD. You create the GitHub Issue on dispatch.
...
- If a task exists in both TASKS.md and as a GitHub Issue, the issue is canonical. TASKS.md entry includes the issue number.
